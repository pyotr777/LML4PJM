...
sub date_to_stddate {
    ...
    if($indate=~/^\d\d\d\d-\d\d-\d\d \d\d:\d\d:\d\d$/) {
        return($indate);
    } elsif($indate=~/^\w\w\w\s+(\w\w\w)\s+(\d+)\s+(\d\d):(\d\d):(\d\d)\s+(\d\d\d\d)$/) {
        ($sec,$min,$hour,$mday,$mon,$year)=($5,$4,$3,$2,$1,$6);
    } elsif($indate=~/^\w\w\w\s+(\w\w\w)\s+(\d+)\s+(\d\d):(\d\d):(\d\d)\s+(CEST)\s+(\d\d\d\d)$/) {
        ($sec,$min,$hour,$mday,$mon,$year)=($5,$4,$3,$2,$1,$7);
    } elsif ($indate=~/^\[(\d\d)\/(\d\d) ([0-9:-]*)\]$/) {
        ($mon,$mday)=($1,$2);        
        my $time=$3;
        my ($sec_c,$min_c,$hours_c,$mday_c,$mon_c,$year,$rest_c)=localtime();
        $year+=1900;
        if ($time=~/^(\d\d):(\d\d):(\d\d)$/) {
            ($hour,$min,$sec)=($1,$2,$3);
        } else {
